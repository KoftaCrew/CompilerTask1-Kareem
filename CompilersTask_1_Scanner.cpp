...
struct Tokenizer
{
    ...
    Tokenizer(CompilerInfo* _info) { info=_info;}
    ...
    void GetNextToken()
    {
        ...
        if(!str) { current_token.type=ENDFILE; return;}

        if(IsLetterOrUnderscore(str[0]))
        {
            for(int i=0; i<num_reserved_words; i++)
                if(StartsWith(str, reserved_words[i].str))
                {
                    current_token=reserved_words[i];
                    info->in_file.Advance(strlen(reserved_words[i].str));
                    return;
                }
        ...
        for(int i=0; i<num_symbolic_tokens; i++)
            if(StartsWith(str, symbolic_tokens[i].str))
            {
                current_token=symbolic_tokens[i];
                info->in_file.Advance(strlen(symbolic_tokens[i].str));
                return;
            }
